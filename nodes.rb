...
class Node
  ...
  def orchestrate_self(delay, conductor, opts)
	...
	def orchestrate(delay, conductor, opts = {})
	  puts (" "*(10-height))+"Orchestrating #{self.class.name}, delay #{delay}"
...
value_only TrueNode do |delay, conductor, opts|
  conductor.enqueue(:sawsaw, [[0, 1], [4, 1], [7, 1], [0, 1]], 4, 4, 4, delay, height)
end

value_only FalseNode do |delay, conductor, opts|
  conductor.enqueue(:sawsaw, [[7, 1], [4, 1], [2, 1], [7, 1]], 4, 4, 4, delay, height)
end

value_only DeleteNode do |delay, conductor, opts| 
  conductor.enqueue(:sawsaw, [[2, 4]], 4, 4, 4, delay, height)
end

value_only ReturnNode do |delay, conductor, opts|
  conductor.enqueue(:sawsaw, [[7, 1], [5, 1], [4, 1], [2, 0.5], [0, 1]], 4, 4, 4, delay, height)
end

value_only TypeOfNode do |delay, conductor, opts|
  conductor.enqueue(:sawsaw, [[0, 0.5], [4, 0.5], [0, 0.5], [4, 0.5], [2, 0.5], [7, 0.5], [2, 0.5], [7,0.5]], 4, 4, 4, delay, height)
end
...
value_only LogicalNotNode do |delay, conductor, opts|
  conductor.enqueue(:guitar, [[0, 1], [0, 1], [7, 1], [7, 1]], 4, 4, 4, delay, height)
end

value_only ThrowNode do |delay, conductor, opts|
  conductor.enqueue(:sawsaw, [[7, 1], [4, 1], [2, 1], [4, 1]], 4, 4, 4, delay, height)
end

value_only UnaryMinusNode do |delay, conductor, opts|
  conductor.enqueue(:sinmix, [[nil, 0.5], [4, 0.5], [nil, 0.25], [5, 0.25], [7, 0.25], [9, 0.25], [nil, 1], [12, 1]], 4, 4, 4, delay, height)
end
